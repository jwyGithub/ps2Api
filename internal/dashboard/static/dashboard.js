...
(function () {
  ...
  // 状态刷新：对统计栏指定状态（exhausted / error / disabled）的所有账号逐个调
  // 单账号额度端点（并发 3）查证额度，每完成一个更新 toast 进度；探测到 AVAILABLE
  // 且余量恢复的账号经 applyUsageState 自动转回 active 入池。showToast 每次调用
  // 重置 2.5s 计时器，天然适合当进度提示。
  window.refreshStatusQuota = function (status) {
    if (state.resetRefreshing) return;
    var list = state.accounts.filter(function (a) { return effectiveStatus(a) === status; });
    if (!list.length) { toast('没有该状态的账号'); return; }
    ...
    var step = function () {
      ...
      var a = list[i];
      toast('刷新 ' + (done + 1) + '/' + list.length + '：' + a.email);
    ...
    Promise.all([step(), step(), step()]).then(function () {
      ...
      var msg = '刷新完成：' + recovered + ' 个成功' + (failed > 0 ? '，' + failed + ' 个失败' : '');
      loadAll().then(function () { toast(msg); });
    });
  };
  // ─── 账号连通性测试（直连 / 网关，完整现场）────────────────
