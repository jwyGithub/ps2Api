...
### 11.2 三次回归：`<imgsrc` 前缀粘连形（2026-09-17 19:59，11.1 文本触发）

**案发**：19:59:34-35 同账号（419）、同 body（91730 字节）2 连败（id 3426-3427，11.1 修复部署后）。经第 12 节 API（sql-query + analyze + 导出 diff）自助排查。

**定位**：CF 归一化穷举 + 3425 成功对照差集，唯一多出的危险形态是 **`<imgsrc=xonerror…`**——11.1 节自己新写的文本：引用归一化形态时写下的**无空格紧凑形**（当时为「忠实展示归一化后的样子」），恰是完整无破坏的 `<img` 标签开头。文档其余 6 处 img 均带 ZWSP/空格已中和，唯此一处裸露。**排查文档引用归一化形态的行为本身成了回归源**。

**规则漏因**：规则 1 的 `\b` 词边界要求标签名后跟非词字符，`<imgsrc` 里 `img` 后是 `s`（词字符）不命中；而 CF 归一化是**纯前缀匹配**（剥空格后 `<imgsrc`、`<imgonerror` 都按 `<img` 前缀命中）。词内连续性由 CF 归一化定义、不由 HTML 规范定义——边界必须跟它对齐。

**修复**（[waf.go](../internal/provider/waf.go) 规则 1 去 `\b`）：标签名**前缀命中即插 ZWSP**。误伤面（`<imgx` 之类非标签词）只多一个不可见字符，模型阅读无损、不破坏 JSON；漏伤代价是 403。权衡下全前缀覆盖。探测表补 `<imgsrc`。单测补 3 用例（`<imgsrc=xonerror`、转义形、`<scriptx=1`）；真实 3427 失败体离线回归零残留。

**文档书写铁律（防第五次）**：本文档引用任何标签/签名形态时，**永远不得出现无破坏的标签前缀**——引用归一化形态须在标签名内插 ZWSP（如 `<img​src` 这种写法本身就是安全的，因为规则前缀命中已把它破坏）或用「img+src」类叙述代替字面量。写完文档后用 `/api/waf/analyze` 对该文档叶子自查一遍签名计数再发布。

### 11.3 四次回归：backtick+curl 命令注入形（2026-09-17 20:15，第 12 节 API 首战）

**案发**：20:15:07-09 同账号（1667）、同 body（103428 字节）4 连败（id 3434-3437，去 `\b` 修复部署后）。**本次排查全程经第 12 节 API 自助完成**（sql-query/analyze/导出体），另加**真实头直连上游重放**：3437 头+3437 体 → 403，3437 头+3425 体 → 200，3425 头+3425 体 → 200——头/Cookie/账号维度彻底排除，403 锁定 body 内容。

**定位**：签名计数全零（新探测表也未命中）+ 解码层归一化穷举**零已知危险形态**——第 12 节方法首次失效，落回**行级二分**（探针变体直接打上游，同第 9/10 节方法论）：152 行文档 → 4 轮收敛到行 34 单行 → 行内继续二分到 16 字符 → ~20 轮探针定案。

**签名**（新发现的第四类，与标签类完全不同）：**backtick/代码围栏上下文里的 `curl`/`wget` + 单连字符 flag + 参数** 触发 CF 命令注入托管规则。实测边界：

| 形态 | 结果 |
|---|---|
| `` `curl -o f url` `` / `` `wget -O f url` `` / `see `curl -L url` here` | **403** |
| ` ``` `（无语言标注）+ `curl -o f url` / ` ``` `+空格+`curl…` | **403** |
| `` ` curl -o f url ``（backtick 空格 curl） | **403** |
| `curl -o f url`（裸，无 backtick） | 放行 |
| `` `curl "y"` ``（无 flag）/ `` `curl -o` ``（flag 无值）/ `curl --header x y`（长 flag）/ `-o-f`（紧连） | 放行 |
| `` `wget -q` ``（flag 无值）/ `` `ls/cat/rm/git/chmod …` `` | 放行 |
| ` ```bash `（有语言标注）+ curl | 放行（防御性起见规则仍覆盖） |

**触发载荷**：第 9 节结论行——`curl -H "Authorization: Bearer …"` 的引用文字（文档描述 bin/cat 案排查时列的「实测放行」项）。**「放行」结论引用了危险形态本身**：CF 规则随时间演进，当时放行的 `curl -H "…"` 如今在 backtick 上下文里 403。与前三次同构的终极形态：**文档记录的实验结论自带时效性，引用旧结论的字面量就是未来的载荷**。

**修复**（[waf.go](../internal/provider/waf.go) 第 7 条规则）：`(`{1,3}`)([^`]{0,40}?)(cu…rl|wg…et)(\s+-[^\s-][^\s]*\s+[^\s])` → backtick 群后（允许 ≤40 字符，覆盖 ` `+curl 与 ` ```\n `+curl）、curl/wget 词内插 ZWSP。实测词内破坏放行（`cu​rl`、`wg​et`、大写、多命令混合、sh 围栏内均 200），flag 无值/长 flag/裸命令不误伤。单测 13+6 用例（命中 7 / 放行 8）；修复后网关端到端重放 3437 客户端请求 → **200**。

**方法论升级（第 12 节流程补强）**：签名计数全零 + 归一化穷举干净 ≠ 内容无辜——**还剩命令注入类**（不依赖标签与归一化，是语义级规则）。第 6 节三分走改为四分：三分走全排除后直接上**行级二分**（第 10 节面板探针或直连重放构造 USER_QUERY），不要再猜签名。
