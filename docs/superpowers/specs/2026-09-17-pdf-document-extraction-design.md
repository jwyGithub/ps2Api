...
- 非 PDF document（text/plain 等）维持 400 拒绝，语义不变。

### 实现
- 新文件 `internal/provider/pdf.go`：`extractPdfText(b64 string) (string, error)`，用 `github.com/ledongthuc/pdf`（纯 Go、无 cgo、无外部依赖）解码 base64 后逐页提取文本。
...
- 端到端：`/v1/messages` 带 PDF document 块不再 400 且块替换为文本；非 PDF document 仍 400。
